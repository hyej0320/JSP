...
<body>
	...
	<ul>
		...
		<li>화면을 구성하는 데 필요한 일부 자바 문법들을 보기 좋은 태그 형식으로 사용할 수 있다</li>
		<li>JSTL은 태그 라이브러리이므로 태그 라이브러리를 import하는 별도의 문법을
			JSP 페이지에 추가해야 사용할 수 있다</li>
		<li>JSTL 다운: Maven Repository에서 검색 후 다운로드</li>
		<li>다운받은 라이브러리를 build path와 Deployment Assembly에 추가해야 한다</li>
	</ul>
	...
	<ul>
		...
		<li>&lt;c:choose&gt;, &lt;c:when&gt;, &lt;c:otherwise&gt;
			 : if, else if, else문을 사용할 수 있는 태그</li>
		<li>&lt;c:forEach&gt; : 반복문을 쉽게 사용할 수 있는 태그</li>
		<li>&lt;c:forToken&gt; : 전달한 문자열을 split하여 반복문을 돌려주는 태그</li>
		<li>&lt;c:url&gt; : contextPath를 추가한 url을 편리하게 생성해주는 태그</li>
	</ul>
	
	<h3># 어트리뷰트에 추가된 값은 편리하게 꺼내 쓸 수 있는 방법이 있다</h3>
	
	<p>편하게 꺼내기 : ${animals} (해당 객체의 toString을 사용함)</p>
	<p>${favSnack}</p>

	<h4>아래의 두 문법은 완전히 똑같은 결과가 된다</h4>
	
	<%pageContext.setAttribute("fruit", "Peach");%>
	<p>기존 자바 문법으로 추가한 값: ${fruit}</p>
	
	<c:set var="fruit" value="복숭아" scope="page"/>
	<p>태그 라이브러리 문법으로 추가한 값: ${fruit}</p>
	
	<h3># 각 영역별 어트리뷰트 추가하기</h3>
	
	<%
		// 같은 이름의 어트리뷰트가 여러 영역에 있다면 가장 가까운(좁은) 범위의 것을 참조한다
		// (application > session > request > pageContext)
		application.setAttribute("coffee", "아메리카노");
		session.setAttribute("coffee", "카페모카");
		request.setAttribute("coffee", "디카페인 아메리카노");
		pageContext.setAttribute("coffee", "초코라떼");
	%>

	<c:set var="footBallPlayer" value="박지성" scope="application"/>
	<c:set var="footBallPlayer" value="황희찬" scope="session"/>
	<c:set var="footBallPlayer" value="김민재" scope="request"/>
	<c:set var="footBallPlayer" value="손흥민" scope="page"/>
	...
	<ul>
		...
		<li>${applicationScope.footBallPlayer}</li>
		<li>${sessionScope.footBallPlayer}</li>
		<li>${requestScope.footBallPlayer}</li>
		<li>${pageScope.footBallPlayer}</li>
	</ul>
	...
	<ul>
		...
		<li>items: 하나씩 꺼낼 수 있는 열거형(enum, iterable한 객체) 객체
			e.g. 배열, 리스트, Set</li>
		<h4>----- 반복문 결과 -----</h4>
	...
	<h3># c:forEach로 숫자를 만들어서 반복돌리기</h3>
	...
	<ul>
		...
		<li>status.content: 현재 반복 횟수 (1부터 시작)</li>
		<li>status.first: 현재 반복이 첫번째인가?</li>
	...
	<c:forEach var="f" items="${fruits}" varStatus="status">
			...
			>${f}(${status.index}번째 아이템, ${status.count}회 반복중)</div>
	</c:forEach>
	...
	<h3>c:url로 url만들어보기</h3>
	<ul>
		<li>절대 경로를 사용할 때는 별 차이가 없지만 
			상대 경로를 사용할 때는 contextPath를 앞에 붙여준다</li>
		...
		<li>c:url에 var을 넣어주면 해당 어트리뷰트에 결과가 저장된다
			(var을 설정하지 않는 경우 그냥 웹페이지에 출력된다)</li>
		<li>
			<c:url var="to_detail" value="/emp/detail">
				<c:param name="employee_id">103</c:param>
				<c:param name="department_id">80</c:param>	
			</c:url>
			<a href="${to_detail}">사원 정보 보러 가기</a>
		</li>
	</ul>
